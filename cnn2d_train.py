...
if len(train_dataset) > 0:
    ...
    print(f"Sample window shape: {sample_window.shape} (expected: [1, num_features, window_size])")
    print(f"Sample label: {sample_label}")
    channels = sample_window.shape[0]
    # Use the second dimension as num_features if available
    num_features = sample_window.shape[1]  
else:
    print("Warning: Train dataset is empty after grouping/filtering.")
    num_features = 72  # fallback
...
# Concatenate predictions, labels, and probabilities from all batches
